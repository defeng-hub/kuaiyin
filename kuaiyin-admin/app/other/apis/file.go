...
func init() {
	envPath = path + config.ApplicationConfig.Mode + "/"
	fmt.Println("env path is :", envPath)
}
...
func (e File) baseImg(c *gin.Context, fileResponse FileResponse, urlPerfix string) FileResponse {
	...
	fileName := guid + ".jpg"
	err := utils.IsNotExistMkDir(envPath)
	if err != nil {
	...
	if source != "1" {
		fileResponse.Path = "/" + envPath + fileName
		fileResponse.FullPath = "/" + envPath + fileName
	}
	return fileResponse
}
...
func (e File) multipleFile(c *gin.Context, urlPerfix string) []FileResponse {
	...
	for _, f := range files {
		...
		multipartFileName := envPath + fileName
