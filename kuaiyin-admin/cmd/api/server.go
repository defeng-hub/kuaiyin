...
func setup() {
	...
	if config.ApplicationConfig.Mode == "lan" || config.ApplicationConfig.Mode == "prod" {
		ftpsync.FtpClientConn = ftpsync.NewFTPClient(ext.ExtConfig.Ftpsync.Host, ext.ExtConfig.Ftpsync.Ftpuser, ext.ExtConfig.Ftpsync.Password)
		go ftpsync.FtpFswatch()
	}
	go queue.Run()
...
func tip() {
	usageStr := `欢迎使用 ` + pkg.Green(`后台管理系统 `+global.Version) + ` 可以使用 ` + pkg.Red(`-h`) + ` 查看命令`
	fmt.Printf("%s \n\n", usageStr)
}
