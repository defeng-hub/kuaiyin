...
func UploadVideo(reader io.Reader, dataLen, userId int64, title string) error {
	...
	//视频文件上传
	err := service.UploadFileWithBytestream("defeng-1253648317", reader, videoName, dataLen, videoContentType)
	if err != nil {
	...
	// 上传第一帧图片
	_ = service.UploadFileWithBytestream("defeng-1253648317", imgReader, imageName, int64(buffer.Len()), imageContentType)
	// 获取第一帧图片的链接
	...
	//插入数据
	_, err = models.InsertVideo(&video)
	//bf.AddVideoId(ID)
